    ...
    unsigned long scope_id, asio::error_code& ec)
{
  ...
  {
    sprintf_s(dest, length_, "%u.%u.%u.%u",
  ...
  {
    size_t n = 0, b = 0, z = 0;
    while (n < length_ && b < 16)
    {
      if (bytes[b] == 0 && bytes[b + 1] == 0 && z == 0)
      {
        do b += 2; while (b < 16 && bytes[b] == 0 && bytes[b + 1] == 0);
        n += sprintf_s(dest + n, length_ - n, ":%s", b < 16 ? "" : ":"), ++z;
      }
      else
      {
        n += sprintf_s(dest + n, length_ - n, "%s%x", b ? ":" : "",
            (static_cast<u_long_type>(bytes[b]) << 8) | bytes[b + 1]);
        b += 2;
      }
    }
    if (scope_id)
      n += sprintf_s(dest + n, length_ - n, "%%%lu", scope_id);
    return dest;
  }
  else
  {
...
inline hostent* gethostbyaddr(const char* addr, int length_, int af,
    hostent* result, char* buffer, int buflength, asio::error_code& ec)
{
  ...
  (void)(buffer);
  (void)(buflength);
  hostent* retval = error_wrapper(::gethostbyaddr(addr, length_, af), ec);
  if (!retval)
    return 0;
  ec = asio::error_code();
  *result = *retval;
  ...
  int error = 0;
  hostent* retval = error_wrapper(::gethostbyaddr_r(addr, length_, af, result,
  ...
  *result = *retval;
  ...
  hostent* retval = 0;
  int error = 0;
  error_wrapper(::gethostbyaddr_r(addr, length_, af, result, buffer,
